...
\section{NUMBER OF PLAYERS}
    ...
    Beware! The rules of the 5–10 \faPeopleGroup\ game are very different from the ones for 2–4 \faPeopleGroup! Some
...
\section{SET-UP}
    ...
    \begin{enumerate}
        ...
        down to each player. Two players will have the boss role and the rest will
        be workers.
        \item Each worker should check the role but keep it secret.
        \item Give a personal token to each worker. Try to find the best suited token for
        each person, so that the association will be easier to remember. Keep aside
        the hourglass token.
        \item Place the Robots (stacked), Public Work, Union Fund, Mayor, and Security
        Guard cards at the centre of the table.
        \item Place the 6 phase cards (on the “5–10 \faPeopleGroup” side) face up one next to the other
        ...
        \item Each Boss starts with 10 \faRubleSign\ coins, and workers with 0 \faRubleSign.
        ...
        the game, players are encouraged to role-play from this point on, starting with each boss
    ...
    \subsubsection{EXAMPLE}
        ...
        \textit{The bosses decide they will compete in the production of spicy watermelon-flavoured
...
\section{VICTORY CONDITIONS}
    ...
    \subsection{BOSS' VICTORY}
        ...
        A boss wins when reaching a certain amount of coins.
    
    \subsection{WORKERS COLLECTIVE VICTORY}
    
        All the workers win collectively when they manage to achieve full automation,
        meaning they manage to expropriate as many robots as the number of workers.
    ...
    \subsection{STARTUPPERS' VICTORY}
        ...
        The worker with the Startupper secret role can win with WORKERS collective victory
        conditions or individually with a Class Betrayal: Accumulating as many
        coins as the total number of players (workers + 2 bosses). The Startupper cannot declare
        the individual victory while Stressed. Also, for reaching the needed
        amount of cash, the Startupper can steal all the coins from the Union Fund card.
    ...
    \subsection{SCABS' VICTORY}
        ...
        Each Scab can win with WORKERS collective victory conditions or together with
        a boss: when a boss declares victory, the Scab also wins if has been working (not
        striking!) for that boss in the same turn (regardless if the Scab is Stressed or not).
    
    \subsection{LUDDITE'S VICTORY}
    
        [6–10 \faPeopleGroup.] The Luddite can win with WORKERS collective victory conditions
        (counting all the expropriated and the destroyed robots) or individually. The
        individual victory happens for the Luddite as soon as 3 robots get destroyed. To
    ...
    \subsection{EARLY ENDING OF THE GAME - MONOPOLY}
        ...
        If a boss is eliminated from the game (i.e. cannot pay Rent, or cannot buy a
        robot after a successful Acceleration Strike held by the workers), a Monopoly is
        formed by the other boss. The workers immediately and automatically expropriate
        the robots of the bankrupted boss.
        In a Monopoly situation, the winner gets determined based on the number of
...
\section{GAMEPLAY}
    ...
    A round consists of 6 phases. Each phase is played a little differently; there is no
    player turn order, and the actions of bosses or workers can take will differ.

    \begin{itemize}
        \item Phase 1 HIRING: bosses compete for workers by offering Contracts; and
        may acquire robots to bolster production. Workers must choose a Contract.
        Players can discuss openly and negotiate.
        \item Phase 2 STRIKE: bosses may compete for the Security Guard. Each worker
        ...
        \item Phase 3 PRODUCTION: bosses gain coins from the Bank, equal to the production
        or their workers and robots.
        \item Phase 4 PAYDAY: bosses may pay their WORKERS salaries.
        \item Phase 5 RENT: both bosses and workers pay their Rent.
        \item Phase 6 UNION FUND: workers may place their coins into the shared Union
        Fund.
    \end{itemize}
...
\section{Phase 1 HIRING}
    ...
    \subsection{BOSSES ACTIONS}
        ...
        \begin{itemize}
            \item May acquire as many robots as they are able to; return 6 \faRubleSign\ coins to the Bank
            (the pile of no one's coins) and place 1 robot in front of them, blue side face
            up.
            \item Must outline their Contract offers - in return for work, how many coins will
            Workers receive. Contracts are not binding. Contracts can be verbal or written,
            the role playing is up to you.
            \item Holidays: in addition to a salary, Contracts may include Holidays. This does
            not have an extra cost for the boss. During phase 4 Payday, Holidays allow
            workers to eliminate Stress. To understand how workers can get Stressed,
            see phase 5 Rent.
        \end{itemize}
    ...
    \subsection{WORKERS ACTIONS}
        ...
        \begin{itemize}
            ...
            \item Must pick who they will work for by giving a boss their personal token.
            \item Depending on the number of players, at most [5–6 \faPeopleGroup] one, or [7–8 \faPeopleGroup] two,
            or [9–10 \faPeopleGroup] three workers may alternatively perform Public Work, by placing their
            personal token on the Public Work card.
            \item Public Work provides some workers (max. 1, 2, or 3 depending on the number
            of players), with a Contract for a single round, paying 1 coin with included Holidays.
        \end{itemize}
    ...
    \subsection{ADDITIONAL DETAILS AND EXAMPLES}
        ...
        \subsubsection{EXAMPLE}
            ...
            \textit{Boss X offers one Contract for 2 \faRubleSign\ and one internship Contract starting from 0 and increasing
...
\section{Phase 2 STRIKE}
    ...
    \subsection{BOSSES ACTIONS}
        ...
        \begin{itemize}
            \item Must bid for the Security Guard, even if their bid is 0, using fingers to
            indicate a number from 0 to 5 during the simultaneous voting.
        \end{itemize}
        
    \subsection{WORKERS ACTIONS}
    
        \begin{itemize}
            \item All non-Stressed workers must decide to strike or work; using, during the
            simultaneous voting, a closed fist to strike or an open hand to work.
        \end{itemize}
    
    \subsection{ALL PLAYERS}

        \begin{itemize}
            \item Simultaneous voting: all the bosses and the non-Stressed workers place
            their hands in the centre of the table, then simultaneously reveal their action.
        \end{itemize}
    
    \subsection{VOTE RESOLUTION}

        \begin{itemize}
            \item First thing after the vote, both bosses pay to the Bank an amount of coins
            equal to their bid multiplied by the number of robots each of them controls.
            ...
            \item The winner of the bidding is the boss with the highest bid: the one who
            indicated the highest number with the fingers, regardless the actually paid
            amount.
            \item Security Guard card: then, the boss who won the bidding, receives the Security
            Guard card for this round. This protects this boss from Expropriation
            ...
            \item For a strike to be successful, the majority of workers must take the strike
            ...
            \item Failed Strike: [5 \faPeopleGroup] In absence of the Luddite role, nothing happens, simply
            the striking workers will not produce during phase 3 Production. [6–10 \faPeopleGroup]
            The Luddite could attack! If at least two workers participated in a failed
            ...
            of the failed strike, may destroy one robot (either assigned to a worker or
            ...
            \item If a strike is successful; workers involved must collectively decide to Expropriate
            a robot or Accelerate the acquisition of a robot. If the workers cannot
            agree on what type of strike they want to carry out, then the bosses decide.
            \item Expropriation strike: workers take a single blue robot owned by a boss
            (excluding the boss whose robots are protected by the Security Guard), flip
            the card to the red side and assign it to a single worker. If the workers
            cannot agree on who keeps the robot, the bosses choose which worker will
            receive it.
            \item Acceleration strike: forcing a single boss to purchase a blue robot instantly,
    ...
    \subsection{ADDITIONAL DETAILS AND EXAMPLES}
        ...
        \begin{itemize}
            \item Stressed workers cannot strike and so must work.
            ...
            \item Workers who strike will not produce during phase 3 Production for their
        ...
        \subsubsection{EXAMPLE: Security Guard}
            ...
            \textit{During the simultaneous voting, boss Y bids 1 with one finger, supposing that it will
        ...
        \subsubsection{EXAMPLE: Strike}
            ...
            \textit{At the same time of the BOSSES bidding, the WORKERS vote happens. Abele, Caino,
        ...
        \subsubsection{EXAMPLE: Sabotage}
            ...
            \textit{[6–10 \faPeopleGroup: the Luddite role is present] ... the next round, boss X has 1 robot and again
...
\section{Phase 3 PRODUCTION}
    ...
    \subsection{BOSSES ACTIONS}
        ...
        \begin{itemize}
            \item Each robot and non-striking worker produces 5 \faRubleSign\ for their boss. Each boss
    ...
    \subsection{ADDITIONAL DETAILS AND EXAMPLES}
        ...
        \subsubsection{EXAMPLE}
            ...
            \textit{Boss X has 1 robot producing 5 and a Contract with Beata who also produces 5.
...
\section{Phase 4 PAYDAY}
    ...
    \subsection{BOSSES ACTIONS}
        ...
        \begin{itemize}
            \item Decide on whether or not to pay their workers as promised in the Contracts
    ...
    \subsection{WORKERS ACTIONS}
        ...
        \begin{itemize}
            \item Each worker who took the Public Work (if any) and did not strike, earns 1
            coin and receive free Holidays.
            \item Also, each Stressed worker can pay for their own Holidays by giving 2 \faRubleSign\ to
            the Bank. They may take this from their own money and/or from the Union
            Fund card (see phase 6 Union Fund).
            \item Holidays allow workers to remove their Stress. To understand how workers
            can get Stressed, see phase 5 Rent. To remove the Stress, Untap the character
    ...
    \subsection{ADDITIONAL DETAILS AND EXAMPLES}
        ...
        \begin{itemize}
            \item The Mayor is a prize lasting one round. Initially, it is placed at the centre
            of the table, not assigned to any boss/worker. It goes to the boss who has
            at least one working (non-striking) worker, respects all the Contracts (excluding
            the striking WORKERS Contracts), and pays at least 1 coin to each
        ...
        \subsubsection{EXAMPLE}
            ...
            \textit{Caino was Stressed, and worked for the Public Work, so he gets directly 1 coin and
...
\section{Phase 5 RENT}
    ...
    \subsection{BOSSES ACTIONS}
        ...
        \begin{itemize}
            \item Each boss must pay a Rent of 2 \faRubleSign\ to the Bank.
        \end{itemize}
    
    \subsection{WORKERS ACTIONS}

        \begin{itemize}
            \item Each worker must pay a Rent of 1 coin to the Bank.
        \end{itemize}
    
    \subsection{ALL PLAYERS}

        \begin{itemize}
            \item The player, worker or boss, who has the Mayor card, if any, can indicate
which players (just one, some of them, or all) will not pay any Rent for this
turn (herself/himself included). Additionally, this player can also increase the
Rent by 1 coin for one player for this turn.
    ...
    \subsection{ADDITIONAL DETAILS AND EXAMPLES}
        ...
        \begin{itemize}
            \item If any worker cannot pay the Rent:
            \begin{itemize}
                \item They may use the coins on the Union Fund card for paying the
                Rent;
                \item If there are no coins on the Union Fund card, the workers who cannot
                pay get Stressed: these players Tap (rotate 90 degrees) their character
            ...
            \end{itemize}
            \item If a boss cannot pay the Rent, see Early ending of the game - Monopoly.
            \item At the end of this phase, return the Mayor card to the centre of the table.
        \end{itemize}
    
        \subsubsection{EXAMPLE}

            \textit{As the new Mayor, the first law issued by boss X is to close down the fine arts museum
...
\section{Phase 6 UNION FUND}
    ...
    \subsection{ADDITIONAL DETAILS}
        ...
        At this point, the game continues with phase 1 and follows all the phases
        again.
